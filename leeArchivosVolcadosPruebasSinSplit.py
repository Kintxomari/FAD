...
def read_valores(filename):
    ...
    #print type (linea)
    tmp = np.float(linea [0:7])
    #print tmp

    i=linea.find(' ',i,len(linea))+2

    cond = np.float(linea [i:i+7]) 
    #print cond 

    i=linea.find(' ',i,len(linea))+1

    hum = np.int_(linea [i:i+3])
    #print hum

    i=linea.find(' ',i,len(linea))+1
 
    ten = np.int_(linea [i:i+3])
    #print ten

    i=linea.find(' ',i,len(linea))+1
    j=linea.find(' ',i+3,len(linea))-1

    tgps =np.float(linea [i:j+1])/1000
    #print tgps

    longrados = np.float(linea[j+2:j+5])
    #print longrados
    #print linea[j+5:j+12]
    londecimasgrado =(np.float(linea[j+5:j+12]))/60 
    ...
    latgrados = np.float(linea[j+14:j+16])
    #print latgrados
    latdecimasgrado =np.float(linea[j+16:j+23])/60

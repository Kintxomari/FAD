...
def main():
    ...
    dir_folder='/home/kintxo/TIC-MOC/Campanya/BoyasInstrumentadas/300234062212150/'
...
#DECLARO UNA VARIABLE DE TIPO DICCIONARIO DONDE VOY A GUARDAR TODOS LOS DATOS
    ...
    datos = {}.fromkeys( ['fecha','latIR','lonIR','numMsg','msg_size','status','humedad','tension','tiempoGPS','latREAL','lonREAL','velocidad','date'] )
...
#CON ESTE FOR INICIALIZO LA VARIABLE
    ...
    for infile in l:
        ...
        # LEO LA FECHA DEL FICHERO .desc
            ...
            string0="grep  Time  /home/kintxo/TIC-MOC/Campanya/BoyasInstrumentadas/300234062212150/" + detalles + "> out0.txt"
            os.system(string0)
       ...
       # LEO LA POSICIÓN DEL FICHERO .desc
            ...
            string2="grep  Lat  /home/kintxo/TIC-MOC/Campanya/BoyasInstrumentadas/300234062212150/" + detalles + "> out2.txt"
            os.system(string2)
      ...
      # LEO EL NÚMERO DE MENSAJE DEL FICHERO .desc
            ...
            string4="grep  MOMSN  /home/kintxo/TIC-MOC/Campanya/BoyasInstrumentadas/300234062212150/" + detalles + "> out4.txt"
            os.system(string4)
      ...
      # LEO EL TAMAÑO DE MENSAJE DEL FICHERO .desc
            ...
            string5="grep  Size  /home/kintxo/TIC-MOC/Campanya/BoyasInstrumentadas/300234062212150/" + detalles + "> out5.txt"
            os.system(string5)
      ...
      # LEO EL STATUS DEl MENSAJE DEL FICHERO .desc
            ...
            string6="grep  Status  /home/kintxo/TIC-MOC/Campanya/BoyasInstrumentadas/300234062212150/" + detalles + "> out6.txt"
            os.system(string6)
       ...
       # LEO TODOS LOS VALORES  DEL FICHERO .sbd,TEMPERATURA,CONDUCTIVIDAD,HUMEDAD,TENSION,TIEMPOGPS Y POSICIÓN REAL
            ...
            string7= """tr "," " " <  /home/kintxo/TIC-MOC/Campanya/BoyasInstrumentadas/300234062212150/""" + posiciones +  "> out7.txt" # quita todas las comnas y las cambia por espacios. Si hago este comando para que no deje nada no funciona ["tr "," ""]
            os.system(string7)
            string8= "tr -s ' ' < out7.txt > out8.txt" # que todos los espacios se quedan en une solo espacio
            os.system(string8) # ejecucion


            humi,tensi,tgpsi,latREALi,lonREALi=read_valores('out8.txt')
...
# ESTAS SON LAS VARIABLES QUE ENVÍA LA BOYA'temperatura','conductividad','humedad','tension','tiempoGPS','latREAL','lonREAL' 
    ...
    print (max(datos['latREAL']))
    print (min(datos['latREAL']))
    print (max(datos['lonREAL']))
    print (min(datos['lonREAL']))
...
def read_valores(filename):
    ...
    tgps =np.float(s[2])/1000
    #print tgps
    if (len(s[3])<11):
        lonreal='nan'
    else:
        longrados = np.float(s[3][0:3])
        #print longrados
        
        londecimasgrado =(np.float(s[3][3:10]))/60 
    ...
    if (len(s[4])<10):
        ...
        latgrados = np.float(s[4][0:2])
        #print latgrados
        latdecimasgrado =np.float(s[4][2:9])/60
    ...
    return  hum,ten,tgps,latreal,lonreal 

################################################################################################
#Plotea las posiciones
################################################################################################

def plotea_posiciones(data):
    ...
    if len(data['lonREAL'])>1: # si el fichero tiene valores validos, len(drifter) > 1 
        ...
        pc=map1.plot(data['lonIR'], data['latIR'],'.',markersize=5,color=('#FF4500')) 
    ...
    # titulo
    title('Posicion',weight='bold',fontsize=15)
...
def guarda_netCDF4(data):
    ...
    wf=Dataset('300234062212150.nc','w',format='NETCDF4')
...
def guardo_diccionario(data):
    ...
    fichero = file("300234062212150.dat", "w") 
